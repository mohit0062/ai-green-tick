...
export default function IndustryDetailClient({ slug, industry }: IndustryDetailProps) {
  const defaultBp = (DEFAULT_FALLBACKS.industry_list || []).find((ind: any) => ind && ind.id === slug) || (DEFAULT_FALLBACKS.industry_list || [])[0] || {}

  // Safe fallback properties
  const title = industry?.title || defaultBp?.title || ''
  const desc = industry?.desc || defaultBp?.desc || ''
  const metric = industry?.metric || defaultBp?.metric || ''
  const iconName = industry?.icon || defaultBp?.icon || ''
  const seoTitle = industry?.seoTitle || defaultBp?.seoTitle || ''
  const seoDescription = industry?.seoDescription || defaultBp?.seoDescription || ''

  const problem = industry?.problemSection || defaultBp?.problemSection
  const features = industry?.features || defaultBp?.features
  const integrations = industry?.integrations || defaultBp?.integrations
  const benchmarkResults = industry?.benchmarkResults || defaultBp?.benchmarkResults
  const caseStudy = industry?.caseStudy || defaultBp?.caseStudy
  const faqs = industry?.faqs || defaultBp?.faqs
                  ...
                  <ul className="space-y-4">
                    {(problem.bullets || []).filter(Boolean).map((bullet: string, idx: number) => (
                      <li key={idx} className="flex items-start gap-3">
                        <span className="flex items-center justify-center size-5 bg-red-100 text-red-600 rounded-full shrink-0 text-xs font-bold font-mono">
                          !
                        </span>
