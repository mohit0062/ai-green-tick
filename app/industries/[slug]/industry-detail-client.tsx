  ...
  const blueprints: Record<string, {
    ...
    ecommerce: {
      ...
      features: [
        ...
        { title: "Instant Payments", desc: "Collect payments securely using Stripe or UPI gateways.", icon: "DollarSign" },
      ...
      useCases: [
        {
          ...
          bullets: ["Send high-resolution architectural brochures as PDF", "99% open rates ensure buyers review materials"],
          bg: "bg-[#EAF3FF]",
...
export default function IndustryDetailClient({ slug, industry }: IndustryDetailProps) {
  const defaultBp = getIndustryBlueprints(slug)

  // Safe section extraction helper
  const getSectionData = (key: 'hero' | 'capabilities' | 'useCases' | 'faqs') => {
    if (industry.sections?.[key]) {
      return industry.sections[key]
    }
    // Backward compatibility with legacy schema
    if (key === 'hero') {
      return {
        title: industry.heroTitle || defaultBp.heroTitle,
        subtitle: industry.subtitle || defaultBp.subtitle,
        customHtml: industry.heroCustomHtml || ''
      }
    }
    if (key === 'capabilities') {
      return {
        cards: (industry.detailedFeatures && industry.detailedFeatures.length > 0) ? industry.detailedFeatures : defaultBp.features,
        customHtml: industry.capabilitiesCustomHtml || ''
      }
    }
    if (key === 'useCases') {
      return (industry.detailedUseCases && industry.detailedUseCases.length > 0) ? industry.detailedUseCases : defaultBp.useCases
    }
    if (key === 'faqs') {
      return {
        items: (industry.faqs && industry.faqs.length > 0) ? industry.faqs : [],
        customHtml: industry.faqsCustomHtml || ''
      }
    }
    return null
  }
  ...
  const capabilitiesSection = getSectionData('capabilities')
  const useCasesList = getSectionData('useCases')
  const faqSection = getSectionData('faqs')

  const faqTabs = [
    {
      name: `${industry.title} FAQs`,
      value: 'general',
      faqs: (faqSection.items || []).map((faq: any, i: number) => ({
        id: `faq-ind-${i}`,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#ECEBE9] text-black font-sans">
      ...
      <main className="flex-grow">
        ...
        {/* 1. HERO SECTION */}
        {heroSection?.customHtml ? (
          <div dangerouslySetInnerHTML={{ __html: heroSection.customHtml }} />
        ) : (
                ...
                <div className="lg:col-span-6 space-y-6 text-left">
                  ...
                  <div className="p-4 border border-[#00b259]/20 bg-[#00b259]/5 rounded-none flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-black text-black">{industry.metric}</div>
                    </div>
                  </div>
                    ...
                    <div className="w-full h-full bg-[#ECEBE9] rounded-[32px] overflow-hidden flex flex-col relative">
                      <div className="bg-[#005c2b] text-white p-3 pt-6 flex items-center gap-2 shrink-0">
                        <div className="size-8 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold font-mono">
                          {industry.title.substring(0, 2).toUpperCase()}
                        </div>
                        <div className="text-left">
                      ...
                      <div className="flex-grow p-3 space-y-3 overflow-y-auto text-left text-[11px] leading-relaxed">
                        {(industry.mockChat || defaultBp.mockChat).map((msg: any, i: number) => (
        ...
        {capabilitiesSection?.customHtml ? (
          <div dangerouslySetInnerHTML={{ __html: capabilitiesSection.customHtml }} />
        ) : (
            ...
            <div className="mx-auto max-w-7xl border-x border-[#C5C4C2] px-4 sm:px-6 lg:px-8 space-y-12">
              ...
              <div className="text-center space-y-4 max-w-3xl mx-auto font-sans">
                ...
                <h2 className="text-2xl sm:text-3xl font-display font-bold text-black leading-snug">
                  Automated use-cases engineered for {industry.title} growth
                </h2>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
                {(capabilitiesSection?.cards || []).map((feat: any, i: number) => {
                  return (
                    <div
                      key={i}
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        <div className="p-2 border border-[#C5C4C2] bg-white group-hover:border-black transition-colors [&>svg]:size-4 flex items-center justify-center">
                          {feat.icon && feat.icon.startsWith('<svg') ? (
                            <div 
                              className="size-4 flex items-center justify-center [&>svg]:size-4"
                              dangerouslySetInnerHTML={{ __html: feat.icon }}
                            />
                          ) : (
                            <LucideIcon name={feat.icon || 'ShoppingBag'} className="size-4 text-[#00b259]" />
                          )}
                        </div>
          ...
          <div className="mx-auto max-w-7xl border-x border-[#C5C4C2] px-4 sm:px-6 lg:px-8 py-16 sm:py-24 space-y-24">
            ...
            <div className="text-center max-w-3xl mx-auto space-y-4">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-black font-display leading-tight">
                Communicate better to support high quality {industry.title} workflows with WhatsApp
              </h2>
            </div>

            <div className="space-y-20">
              {(useCasesList || []).map((uc: any, i: number) => {
                if (uc.customHtml) {
                  return <div key={i} dangerouslySetInnerHTML={{ __html: uc.customHtml }} />
                }
                ...
                return (
                  <div key={i} className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                    ...
                    {/* Graphic block */}
                    <div className={`lg:col-span-6 ${uc.bg || 'bg-[#EAF3FF]'} border border-[#C5C4C2] p-8 sm:p-12 flex flex-col justify-center items-center rounded-none shadow-sm aspect-video ${isEven ? 'order-2' : 'order-2 lg:order-1'}`}
        ...
        {/* 4. FAQ SECTION */}
        {faqSection?.customHtml ? (
          <div dangerouslySetInnerHTML={{ __html: faqSection.customHtml }} />
        ) : (
          <div id="faq">
            <FAQ tabs={faqTabs} />
          </div>
