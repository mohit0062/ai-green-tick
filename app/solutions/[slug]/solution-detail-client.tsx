...
export default function SolutionDetailClient({ slug, solution }: SolutionDetailProps) {
  // ── Fallbacks ─────────────────────────────────────────────────────────────
  const badgeText = solution.hero?.badgeText || 'SOLUTION BLUEPRINT'
  const heroHeading = solution.hero?.heading || solution.title || 'Dynamic Solution Page'
  const heroSubheading = solution.hero?.subheading || solution.description || solution.shortDesc || ''
  
  const mockChat = solution.hero?.mockChat && solution.hero.mockChat.length > 0
    ? solution.hero.mockChat
    : [
        { sender: 'user', text: `How can this ${solution.title} solution help our business?` },
        { sender: 'bot', text: `It automates customer updates and coordinates workflows directly on WhatsApp! 🚀` }
      ]

  const capabilities = solution.capabilities && solution.capabilities.length > 0
  ...
  const useCases = solution.useCases && solution.useCases.length > 0
    ? solution.useCases
    : [
        {
          title: `Scale ${solution.title} Customer Relations`,
  ...
  const faqsList = solution.faqs && solution.faqs.length > 0
    ? solution.faqs
    : [
        {
          question: `How long does it take to implement this ${solution.title} solution?`,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center pb-8">
                ...
                <div className="flex items-center gap-2">
                  <div className="p-2 border border-[#C5C4C2] bg-white text-black size-9 flex items-center justify-center shrink-0 [&>svg]:size-5">
                    {solution.icon && solution.icon.startsWith('<svg') ? (
                      ...
                      <LucideIcon name={solution.icon || 'Globe'} className="size-5 text-[#00b259]" />
                    )}
                  </div>
                ...
                {solution.metric && (
                  <div className="p-4 border border-[#00b259]/20 bg-[#00b259]/5 rounded-none flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-black text-black">{solution.metric}</div>
                    </div>
                  </div>
                )}
              ...
              <div className="lg:col-span-6 flex justify-center lg:justify-end w-full">
                {solution.hero?.imageUrl ? (
                  ...
                  >
                    {solution.hero.imageLink ? (
                      <Link href={solution.hero.imageLink} target="_blank" className="block w-full h-full">
                        <img 
                          src={solution.hero.imageUrl} 
                          alt={solution.title} 
                    ...
                    <div className="w-full h-full bg-[#ECEBE9] rounded-[32px] overflow-hidden flex flex-col relative">
                      <div className="bg-[#005c2b] text-white p-3 pt-6 flex items-center gap-2 shrink-0">
                        <div className="size-8 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold font-mono">
                          {solution.title.substring(0, 2).toUpperCase()}
                        </div>
                        <div className="text-left">
          ...
          <div className="mx-auto max-w-7xl border-x border-[#C5C4C2] px-4 sm:px-6 lg:px-8 space-y-12">
            ...
            <div className="text-center space-y-4 max-w-3xl mx-auto font-sans">
              ...
              <h2 className="text-2xl sm:text-3xl font-display font-bold text-black leading-snug">
                Powering operations with robust, scalable solutions
              </h2>
            </div>
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="p-2 border border-[#C5C4C2] bg-white group-hover:border-black transition-colors [&>svg]:size-4 flex items-center justify-center">
                      <LucideIcon name={cap.icon || 'Globe'} className="size-4 text-[#00b259]" />
                    </div>
                    <span className="text-[10px] font-mono font-bold text-neutral-400">[ 0{i + 1} ]</span>
                  </div>
                  <div className="text-left space-y-1 mt-4">
                    <h3 className="text-xs font-black uppercase text-black">{cap.title}</h3>
                    <p className="text-[11px] text-neutral-500 leading-normal font-sans">{cap.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mx-auto max-w-7xl border-x border-[#C5C4C2] px-4 sm:px-6 lg:px-8 py-16 sm:py-24 space-y-24">
            ...
            <div className="text-center max-w-3xl mx-auto space-y-4">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-black font-display leading-tight">
                Streamline operations and elevate outcomes using {solution.title}
              </h2>
            </div>
