...
export default function SolutionForm({ initialSolution, onSave, onCancel }: SolutionFormProps) {
  ...
  const [description, setDescription] = useState(initialSolution?.description || '')
  ...
  const [icon, setIcon] = useState(initialSolution?.icon || 'Globe')
  const [link, setLink] = useState(initialSolution?.link || '')
  const [previewType, setPreviewType] = useState(initialSolution?.previewType || 'whatsapp-solution')
  ...
  // Hero Section
  const [heroBadgeText, setHeroBadgeText] = useState(initialSolution?.hero?.badgeText || (initialSolution ? 'SOLUTION BLUEPRINT' : ''))
  const [heroHeading, setHeroHeading] = useState(initialSolution?.hero?.heading || initialSolution?.title || '')
  const [heroSubheading, setHeroSubheading] = useState(
    initialSolution?.hero?.subheading || initialSolution?.description || initialSolution?.shortDesc || ''
  )
  const [heroMockChat, setHeroMockChat] = useState<{ sender: 'user' | 'bot'; text: string }[]>(
    initialSolution?.hero?.mockChat && initialSolution.hero.mockChat.length > 0
      ? initialSolution.hero.mockChat
      : initialSolution
      ? [
          { sender: 'user', text: `How can this ${initialSolution.title} solution help our business?` },
          { sender: 'bot', text: `It helps streamline messaging and customer communications for your industry! 🚀` }
        ]
      : []
  )
  const [heroImageUrl, setHeroImageUrl] = useState(initialSolution?.hero?.imageUrl || '')
  const [heroImageLink, setHeroImageLink] = useState(initialSolution?.hero?.imageLink || '')

  // Capabilities
  const [capabilities, setCapabilities] = useState<{ title: string; desc: string; icon: string }[]>(
    initialSolution?.capabilities && initialSolution.capabilities.length > 0
      ? initialSolution.capabilities
      : initialSolution
      ? [
          { title: 'Industry Specific Flow', desc: 'Pre-configured workflow templates matching standard operations.', icon: 'ShieldCheck' },
          { title: 'WhatsApp Broadcasting', desc: 'Acquire and retarget users with highly-segmented campaigns.', icon: 'Megaphone' },
          { title: 'Interactive Chatbots', desc: 'Direct visitors automatically based on keyword tags.', icon: 'Cpu' }
        ]
      : []
  )
  ...
  const [useCases, setUseCases] = useState<{
    ...
    initialSolution?.useCases && initialSolution.useCases.length > 0
      ? initialSolution.useCases
      : initialSolution
      ? [
          {
            title: `Scale ${initialSolution.title} Customer Relations`,
            desc: `Configure templates and triggers dynamically to boost user conversions.`,
            bullets: ['98% open rates compared to email', 'Boost click-through rate by 3x'],
            bg: 'bg-[#EAF3FF]',
            chat: [`Need support? Click buttons to choose industry categories.`, `Found what I wanted immediately!`],
            imageUrl: ''
          }
        ]
      : []
  ...
  const [faqs, setFaqs] = useState<{ question: string; answer: string }[]>(
    initialSolution?.faqs && initialSolution.faqs.length > 0
      ...
      ? [
          {
            question: `How long does it take to implement this ${initialSolution.title} solution?`,
            answer: `It takes less than 15 minutes to configure the defaults and go live using official WhatsApp APIs.`
          },
          {
            question: `Does this integrate with Shopify or standard CRMs?`,
            answer: `Yes! Our platform supports native integrations with Shopify, WooCommerce, HubSpot, and custom webhooks.`
          }
        ]
      : []
  )
  ...
  // Content AI Smart Generation (Claude via API)
  const handleAIGenerate = async () => {
    ...
    try {
      const systemPrompt = `You are a senior marketing copywriter for AI Greentick, a WhatsApp Business API SaaS platform. You write conversion-focused, professional solutions page content. Always respond ONLY with valid JSON, no markdown, no extra text.`

      const userPrompt = `Generate complete solutions page content for a WhatsApp API vertical/solution called "${title}".
...
Return a JSON object with EXACTLY these keys:
{
  ...
  "heroBadgeText": "UPPERCASE 2-word badge (e.g. ECOMMERCE SALES, FINANCE OPS)",
  "heroHeading": "Compelling hero H1 heading (8-12 words)",
  "heroSubheading": "Supporting subtitle (15-25 words)",
  "heroMockChat": [
    {"sender": "user", "text": "User message relevant to the solution"},
    {"sender": "bot", "text": "Bot reply showing the solution value"},
  ...
  const handleInlineAI = async (fieldName: string, currentValue: string, setter: (val: string) => void) => {
    try {
      ...
      const prompt = `The solution is called "${title}". Generate a professional, conversion-focused ${fieldName} for this solutions page.${currentValue ? ` Current text: "${currentValue}". Improve it.` : ''} ${aiInstructions ? `Additional context: ${aiInstructions}` : ''}`
  ...
  const [shortDesc, setShortDesc] = useState(initialSolution?.shortDesc || '')
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12 font-sans text-neutral-800 antialiased select-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {/* WordPress Content Editor Layout */}
          <div className="relative">
            <RichEditor
              value={description}
              onChange={setDescription}
              placeholder="Start writing solution details content here..."
              minHeight="260px"
              showAddMedia={true}
            />
            <button
            ...
            <CardContent className="p-5 space-y-4 text-xs text-neutral-700 bg-white font-normal">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-1.5">
                  <Label htmlFor="icon" className="text-xs font-semibold text-neutral-750">Lucide Icon (e.g. Globe, Smartphone, Heart)</Label>
                  <Input 
                    id="icon" 
                    value={icon} 
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    <option value="whatsapp-solution">WhatsApp Solution Preview</option>
                    <option value="ecommerce-solution">E-Commerce Solution</option>
                    <option value="healthcare-solution">Healthcare Solution</option>
                    <option value="education-solution">Education Solution</option>
                    <option value="realestate-solution">Real Estate Solution</option>
            ...
            <CardContent className="p-5 space-y-4 text-xs text-neutral-700 bg-white font-normal">
              <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  placeholder="e.g. SOLUTIONS BLUEPRINT" 
              ...
              <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  placeholder="e.g. WhatsApp API solutions tailored for your business needs" 
                  className="border-neutral-300 h-9 bg-white font-normal"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <RichEditor
                  ...
                  placeholder="Acquire, qualify, and assist visitors with custom WhatsApp templates and automated replies."
                  minHeight="100px"
                />
              </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2.5">
                    ...
                    {heroImageUrl ? (
                      <div className="flex items-center gap-3 p-2.5 border border-neutral-200 rounded-lg bg-neutral-50/50">
                        ...
                        <Button
                          ...
                          className="h-7 text-[10px] text-red-500 hover:text-red-650 hover:bg-red-55 cursor-pointer border-neutral-300 font-bold shrink-0"
                        >
                          Remove
                        </Button>
                      </div>
                    ...
                    <div className="space-y-1">
                      <Label className="text-[10px] font-semibold text-neutral-600">Or Paste Direct Image Link</Label>
                      <Input 
                        value={heroImageUrl} 
                  ...
                  <div className="space-y-2.5">
                    ...
                    <p className="text-[9px] text-neutral-400 font-normal leading-normal">
                      Optional. Enter a webpage URL here if you want visitors to be redirected when they click on the mockup image.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="p-5 space-y-3">
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                  ...
                  <div key={idx} className="p-3 bg-neutral-50/40 border border-neutral-200 rounded-lg space-y-2 relative">
                    <div className="flex justify-between items-center">
                      ...
                      <Button
                        ...
                        className="h-7 w-7 text-red-500 hover:text-red-650 hover:bg-red-55 cursor-pointer"
                      >
                        <Trash2 className="h-3.5 w-3.5" />
                      </Button>
                    </div>
                ...
                {capabilities.length === 0 && (
                  <p className="text-xs text-neutral-455 italic text-center p-6 border border-dashed rounded-md bg-neutral-50/10">
          ...
          <Card className="border border-[#C5C4C2]/50 shadow-xs bg-white rounded-lg">
            <CardHeader className="bg-neutral-50/50 border-b border-[#C5C4C2]/40 py-3.5 px-5 flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-[10px]">Configure scenario blocks or workflow steps showing how this solution works.</CardDescription>
              </div>
              <Button 
                type="button" 
                size="sm" 
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          placeholder="Automated customer onboarding"
                          className="bg-white h-8 text-xs border-neutral-300"
                        />
                      </div>
                      ...
                      {uc.bullets.map((bullet, bIdx) => (
                        <div key={bIdx} className="flex gap-2 items-center">
                          ...
                          <Button
                            ...
                            className="h-7 w-7 text-neutral-450 hover:text-red-500 cursor-pointer"
                          >
                            <X className="h-3 w-3" />
                          </Button>
                        </div>
            ...
            <CardContent className="p-5 space-y-4 text-xs text-neutral-700 bg-white font-normal">
              <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  placeholder={`${title || 'Solution'} | AI Greentick`} 
                  className="border-neutral-300 h-9 bg-white font-normal"
                />
              ...
              <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  placeholder="whatsapp ecommerce, support solution, customer relations" 
                  className="border-neutral-300 h-9 bg-white font-normal"
                />
              </div>
            ...
            <CardHeader className="bg-neutral-50/50 border-b border-[#C5C4C2]/40 py-3.5 px-4 flex flex-row items-center justify-between">
              ...
              <Badge
                className={cn(
                  ...
                  "bg-red-55 text-red-600 border-red-400"
                )}
              >
              ...
              {seoChecklist.checks.map((check, i) => (
                <div key={i} className="flex items-center gap-2 py-1 px-1 rounded hover:bg-neutral-50 transition-colors" title={check.tip}>
                  ...
                  <span className={cn(
                    ...
                    check.pass ? "text-neutral-600" : "text-red-650"
                  )}>
                    {check.label}
                  </span>
                </div>
