...
export default function SolutionsManagerClient({ initialSolutions }: SolutionsManagerClientProps) {
  ...
  const normalizeSolutions = (sols: Solution[]): Solution[] => {
    ...
    return sols
      ...
      .map((sol, index) => {
        ...
        const shortDesc = sol.shortDesc || sol.desc || ''
        const description = sol.description || sol.desc || shortDesc

        return {
          ...sol,
          id: sol.id || `solution-${index}`,
          title,
          desc: sol.desc || shortDesc,
          shortDesc,
          description,
  ...
  const handleOpenQuickEdit = (sol: Solution) => {
    ...
    setQuickEditForm({
      ...
      metric: sol.metric || '',
      shortDesc: sol.shortDesc || sol.desc || ''
    })
  }
    ...
    const updated = solutions.map(sol => {
      if (sol.id === quickEditSolution.id) {
        return {
          ...
          desc: quickEditForm.shortDesc.trim(),
          shortDesc: quickEditForm.shortDesc.trim(),
          description: sol.description || quickEditForm.shortDesc.trim(),
  ...
  const filteredSolutions = solutions.filter(sol => {
    ...
    if (query) {
      ...
      const matchDesc = (sol.description || sol.desc || sol.shortDesc || '').toLowerCase().includes(query)
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 font-sans text-black relative select-none">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-neutral-200 pb-5">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-neutral-800">AIS Solutions Page</h2>
      ...
      <Card className="shadow-sm border border-neutral-200">
        <CardHeader className="pb-3 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <CardTitle>Solutions Directory</CardTitle>
