...
import { TrendingUpIcon, HeadsetIcon, BellRingIcon, RefreshCcwDotIcon, ClipboardIcon, FileTextIcon, ArrowUpRightIcon, DollarSignIcon } from 'lucide-react'
  ...
  return (
    <section className='relative flex flex-col overflow-hidden'>
      <div className='border-b px-4 sm:px-6 lg:px-8'>
        <div className='mx-auto flex max-w-7xl flex-col gap-6 border-x px-4 py-8 sm:px-6 sm:py-16 md:gap-8 lg:px-8 lg:py-24'>
          <div className='flex flex-col items-center gap-4 text-center'>
            <Badge variant='outline' className='bg-muted relative h-auto gap-2.5 px-1.5 py-1'>
            ...
            <div className='flex flex-wrap items-center justify-center gap-4'>
              <PrimaryOrionButton className='rounded-lg max-[425px]:has-[>svg]:px-4' asChild>
                <a href='#'>
                  <ArrowUpRightIcon />
                  Get Started
                </a>
              </PrimaryOrionButton>
              <SecondaryOrionButton className='rounded-lg max-[425px]:has-[>svg]:px-4' asChild>
                <a href='#'>
                  <DollarSignIcon />
                  View Pricing
                </a>
          ...
          <div className='flex w-full items-center justify-center gap-4 max-sm:flex-col sm:gap-7'>
            ...
            <Separator orientation='vertical' className='data-vertical:h-4 data-vertical:self-center max-sm:hidden' />
              ...
              <TabsList className='h-fit w-full -space-x-px rounded-none bg-transparent p-0 group-data-horizontal/tabs:h-fit'>
                {tabs.map(({ icon, name, value }) => (
                  <TabsTrigger
                    key={value}
                    value={value}
                    className='border-border text-foreground focus-visible:outline-primary/20 data-active:border-primary/60! data-active:bg-muted! h-15 flex-1 cursor-pointer rounded-none px-4 py-2.5 text-base group-data-horizontal/tabs:after:h-0 focus-visible:ring-0 focus-visible:outline-[3px] focus-visible:-outline-offset-4 data-active:z-1'
                  >
                    {icon}
                    {name}
                  </TabsTrigger>
