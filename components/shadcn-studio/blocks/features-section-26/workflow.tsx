...
import { MoreVerticalIcon, ChevronDownIcon, CheckIcon, PaperclipIcon, Loader2Icon, ArrowUpRightIcon, FileDownIcon } from 'lucide-react'
...
const Workflow = ({
  ...
  const fullText = 'Broadcast discount campaign to all contacts and sync with CRM'
  ...
  const handleReviewAndRefine = () => {
    ...
    setInputValue('Campaign broadcast and CRM sync completed successfully')
    setIsLoading(false)
    setShowConnecting(false)
  ...
  useEffect(() => {
    ...
    const getStartingStep = () => {
      if (activeTab === 'sales') return 2
      if (activeTab === 'support') return 3

      return 1 // initialTabId (marketing)
    }
  ...
  return (
    <div
      className={cn(
        'bg-background relative z-1 w-full max-w-99 flex-col rounded-xl shadow-lg border',
